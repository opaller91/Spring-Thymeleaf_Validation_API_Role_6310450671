...
public class MenuService {
    ...
    @Autowired //เพื่อส่งต่อให้ menuapi
    private RestTemplate restTemplate;

    @Autowired //เพื่อขอtoken
    private JwtAccessTokenService tokenService;

    public List<MenuDto> getMenus() { //การ GET
        ...
        ResponseEntity<MenuDto[]> response =
                restTemplate.exchange(url, HttpMethod.GET,
                        entity, MenuDto[].class);//รับกลับมาเป็น arrayของMenuDto

        MenuDto[] menus = response.getBody();
        return Arrays.asList(menus);
    }

    public MenuDto addMenu(MenuDto menu) {//การ POST
